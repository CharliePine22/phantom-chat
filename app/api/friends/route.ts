...
export async function POST(
  ...
  // Throw an error if there are no users with the submitted name
  ...
  if(otherUserFriendsRequestsList.includes(currentUser.id)) {  
...
if(body.method == 'ACCEPT') {
  ...
  const acceptFriendRequest = await prisma.user.update({
    ...
    data: {
      friendRequests: {
        set: currentUser?.friendRequests.filter(request => request !== friendId)
      },
      friendsList: {
        push: friendId
      }
...
const updateFriendsLists = await prisma.$transaction([acceptFriendRequest, updateOtherUser])
